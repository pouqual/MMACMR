...
class RecipeTransformerEncoder(nn.Module):
    ...
    def forward(self, input, name=None):
        ...
        if len(input.size()) == 2:
            ...
            # reshape back
            out = out.view(input.size(0), input.size(1), out.size(-1))  #64x18x512, 64x19x512

            # create mask for second transformer
            attn_mask = input > 0       #64x18x15, 64x19x15
            mask_list = (attn_mask.sum(dim=-1) > 0).bool()  #64x18, 64x19
